...
parser=argparse.ArgumentParser();parser.add_argument("--out",required=True);parser.add_argument("--views",default="")
...
for camera in cameras:
    ...
    records.append({"id":camera.name[7:],"position":list(camera.location),"rotation":list(camera.rotation_euler),"fovRadians":camera.data.angle})
(out/"source-render-settings.json").write_text(json.dumps({"engine":"CYCLES","samples":64,"width":1280,"height":800,"look":scene.view_settings.look,"exposure":scene.view_settings.exposure,"views":records},indent=2)+"\n")
