...
def main():
    ...
    for obj in meshes:
        if obj.get("vrataNavigableBoundsPolicy")=="exclude":obj["vrataExcludeFromSceneBounds"]=True
    ...
    image.name="lightmap-0.4.0-pipeline-median5"
    ...
    image.filepath="//textures/lightmap-0.4.0-pipeline-median5.png"
    if saved:
    ...
    for texture in bpy.data.images:
        if texture.users and texture != image and not texture.name.startswith("cannon-4k") and max(texture.size) > 512:
