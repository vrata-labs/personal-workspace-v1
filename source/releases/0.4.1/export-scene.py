"""0.4.1 atlas/export: preserve source normals and export object-level assemblies.

Constituent geometry remains in saved source for contact checks. Joining happens
only after saving, records every constituent ID, and does not decimate geometry.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    for texture in bpy.data.images:
        if texture.users and texture != image and not texture.name.startswith(("cannon-4k", "book-spines")) and max(texture.size) > 512:
    ...
    exported = []
    for obj in objects:
        if obj.name in ("room-shell.floor", "workspace-main.face"):
            # Keep interaction/capture-sensitive part identities explicit.
            exported.append(obj)
            continue
        groups.setdefault(obj["vrataObjectId"], []).append(obj)
    for key, members in sorted(groups.items()):
        if len(members) == 1:
            exported.append(members[0])
            continue
        bpy.ops.object.select_all(action="DESELECT")
        for obj in members:
            obj.select_set(True)
        ...
        obj.name = f"{key}.assembly"
        obj["vrataPartId"] = "assembly"
        obj["vrataConstituentParts"] = json.dumps(names, separators=(",", ":"))
        exported.append(obj)
    bpy.ops.object.select_all(action="DESELECT")
    for obj in exported:
        obj.select_set(True)
    ...
    print(json.dumps({"glbBytes": output.stat().st_size, "sha256": hashlib.sha256(output.read_bytes()).hexdigest(),
                      "sourceParts": len(objects), "exportedAssemblies": len(exported)}), flush=True)
