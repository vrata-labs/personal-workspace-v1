...
import { BAKED_RELEASE, BAKED_RELEASE_VERSION, REVIEW_VIEWS, assert, assertScratchOutput, fileRecord, readJson } from "./lib.mjs";
import { FINAL_TECHNICAL_VISUAL_PARITY_POLICY } from "./visual-parity-config.mjs";

const root = resolve(import.meta.dirname, "..");
const outputDir = resolve(root, BAKED_RELEASE.runtimeCapturePath);
const reportPath = resolve(root, process.env.SCENE_HISTORICAL_VISUAL_REPORT_PATH ?? `build/visual-parity-${BAKED_RELEASE_VERSION}.json`);
...
process.stdout.write(`Historical technical visual parity passed: PHASH total ${phashTotal.toFixed(5)}, NCC mean ${nccMean.toFixed(8)}\n`);
